...
sub initPlugin {
    ...
    if ( $Foswiki::Plugins::VERSION < 1.026 ) {
        Foswiki::Func::writeWarning(
            "Version mismatch between AutoTemplatePlugin and Plugins.pm");
        return 0;
    }

    # get configuration
    my $modeList = $Foswiki::cfg{Plugins}{AutoTemplatePlugin}{Mode}
      || "rules, exist";
...
# SMELL: templatetopic and formtemplate from url params come into play here as well
    ...
    if ($debug) {
        if ($currentTemplate) {
            if ($override) {
                writeDebug(
                    "$templateVar already set, overriding with: $templateName");
            }
            else {
...
sub _getTemplateFromSectionInclude {
    ...
    writeDebug(
        "called _getTemplateFromSectionInclude($formName, $topic, $web)");
...
sub _getTemplateFromTemplateExistence {
    ...
    writeDebug(
        "called _getTemplateFromTemplateExistence($formName, $topic, $web)");
